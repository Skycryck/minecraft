...
// Labels FR uniquement pour les types de déplacement (UI)
// Les noms de blocs/items/mobs restent en anglais (formatés automatiquement)
const LABELS = {{
  walk:'Marche',sprint:'Sprint',swim:'Nage',fly:'Vol créatif',aviate:'Elytra',boat:'Bateau',
...
function getArchetype(p){{
  if(p.play_hours<1)return{{name:'Nouveau',icon:'🌱',color:'var(--text-muted)'}};
  ...
  const types={{
    miner:{{name:'Mineur',icon:'⛏',color:'var(--yellow)'}},
    fighter:{{name:'Combattant',icon:'⚔',color:'var(--red)'}},
    explorer:{{name:'Explorateur',icon:'🧭',color:'var(--green)'}},
    builder:{{name:'Bâtisseur',icon:'🏗',color:'var(--cyan)'}},
    farmer:{{name:'Fermier',icon:'🌾',color:'var(--orange)'}}
  }};
...
function getFunFacts(name,p){{
  ...
  if(ek>50)facts.push({{score:ek/_maxOf(p=>p.killed_top10?.enderman||0),emoji:'👁',text:`A tué ${{fmt(ek)}} Endermen — environ ${{Math.floor(ek/64)}} stacks d'Ender Pearls`}});

  if(p.deaths>3&&h>1){{const mpd=Math.round(h*60/p.deaths);const deathRate=p.deaths/h;
    facts.push({{score:deathRate/_maxOf(p=>p.play_hours>1?p.deaths/p.play_hours:0),emoji:'💀',text:`Meurt en moyenne toutes les ${{mpd}} minutes`}})}}

  const wk=p.distances?.walk||0;
  if(wk>20)facts.push({{score:wk/_maxOf(p=>p.distances?.walk||0),emoji:'🏃',text:`A marché ${{wk.toFixed(0)}} km — soit ${{(wk/42.195).toFixed(1)}} marathons`}});

  if(p.jumps>500&&h>1)facts.push({{score:p.jumps/_maxOf(p=>p.jumps||0),emoji:'🦘',text:`Saute ${{Math.round(p.jumps/h)}} fois par heure en moyenne`}});

  if(p.total_mined>500&&h>1)facts.push({{score:p.total_mined/_maxOf(p=>p.total_mined||0),emoji:'⛏',text:`Mine ${{(p.total_mined/(h*60)).toFixed(1)}} blocs par minute en moyenne`}});

  if(p.sleep_in_bed>5)facts.push({{score:p.sleep_in_bed/_maxOf(p=>p.sleep_in_bed||0),emoji:'🛏',text:`A dormi ${{p.sleep_in_bed}} nuits Minecraft — ${{(p.sleep_in_bed/h).toFixed(1)}} par heure de jeu`}});

  if(p.open_chest>100)facts.push({{score:p.open_chest/_maxOf(p=>p.open_chest||0),emoji:'📦',text:`A ouvert ${{fmt(p.open_chest)}} coffres — un vrai fouineur`}});

  const pvpD=(p.killed_by?.player)||0;
  if(pvpD>3)facts.push({{score:pvpD/_maxOf(p=>(p.killed_by?.player)||0),emoji:'🎯',text:`Tué ${{pvpD}} fois par d'autres joueurs — cible favorite du serveur`}});

  const ely=p.distances?.aviate||0;
  if(ely>50)facts.push({{score:ely/_maxOf(p=>p.distances?.aviate||0),emoji:'🪂',text:`${{ely.toFixed(0)}} km en Elytra — l'équivalent de ${{Math.round(ely/6)}} traversées de Paris`}});

  if(p.damage_dealt>10000)facts.push({{score:p.damage_dealt/_maxOf(p=>p.damage_dealt||0),emoji:'💔',text:`A infligé ${{fmt(Math.round(p.damage_dealt/20))}} cœurs de dégâts au total`}});

  if(p.traded_with_villager>50)facts.push({{score:p.traded_with_villager/_maxOf(p=>p.traded_with_villager||0),emoji:'🤝',text:`${{fmt(p.traded_with_villager)}} échanges avec des villageois — le capitaliste du serveur`}});

  const brk=Object.values(p.broken||{{}}).reduce((s,v)=>s+v,0);
  if(brk>5)facts.push({{score:brk/_maxOf(p=>Object.values(p.broken||{{}}).reduce((s,v)=>s+v,0)),emoji:'💥',text:`A cassé ${{brk}} outils — pas très soigneux`}});

  if(p.mob_kills>100)facts.push({{score:p.mob_kills/_maxOf(p=>p.mob_kills||0),emoji:'⚔',text:`${{fmt(p.mob_kills)}} mobs tués — soit ${{Math.round(p.mob_kills/h)}} par heure`}});

  if(p.animals_bred>20)facts.push({{score:p.animals_bred/_maxOf(p=>p.animals_bred||0),emoji:'💕',text:`A élevé ${{p.animals_bred}} animaux — fermier dans l'âme`}});

  if(p.fish_caught>10)facts.push({{score:p.fish_caught/_maxOf(p=>p.fish_caught||0),emoji:'🎣',text:`A pêché ${{p.fish_caught}} poissons — le pêcheur du serveur`}});

  const totalDist=p.total_distance_km;
  if(totalDist>100)facts.push({{score:totalDist/_maxOf(p=>p.total_distance_km||0),emoji:'🌍',text:`${{totalDist.toFixed(0)}} km parcourus au total — l'équivalent d'un ${{totalDist>1000?'Paris-Barcelone':'Paris-Londres'}}`}});

  // Tri par score décroissant et top 5
...
function buildTreemapHtml(entries){{
  if(!entries.length)return '<div style="color:var(--text-muted);padding:1rem;font-family:var(--font-mono);font-size:.8rem">Aucun bloc miné</div>';
...
function buildBrokenHtml(broken){{
  ...
  if(!entries.length)return '<div style="color:var(--text-muted);font-family:var(--font-mono);font-size:.8rem">Aucun outil cassé</div>';
...
function buildOverview(){{
  ...
  <div class="section active" id="overview">
    <div class="grid grid-4" style="margin-bottom:1rem">
      <div class="stat-tile"><div class="value" style="color:var(--accent-light)" data-target="${{totalHours.toFixed(0)}}" data-suffix="h">0</div><div class="label">Temps de jeu total</div></div>
      <div class="stat-tile"><div class="value" style="color:var(--green)" data-target="${{totalMined}}">0</div><div class="label">Blocs minés</div></div>
      <div class="stat-tile"><div class="value" style="color:var(--red)" data-target="${{totalKills}}">0</div><div class="label">Mobs tués</div></div>
      <div class="stat-tile"><div class="value" style="color:var(--orange)" data-target="${{totalCrafted}}">0</div><div class="label">Items craftés</div></div>
    </div>
...
const TIER_LABELS=['🔒','🥉 Bronze','🥈 Argent','🥇 Or','💎 Diamant'];
const TIER_COLORS=['var(--text-muted)','#cd7f32','#c0c0c0','#ffd700','#b9f2ff'];
const TIER_EMOJIS=['🥉','🥈','🥇','💎'];
const BADGE_CATEGORIES=[
  {{id:'mining',name:'⛏ Minage'}},
  {{id:'combat',name:'⚔ Combat'}},
  {{id:'survival',name:'💀 Survie'}},
  {{id:'exploration',name:'🚶 Exploration'}},
  {{id:'farming',name:'🌾 Farming & Économie'}},
  {{id:'craft',name:'🔨 Craft & Technique'}},
  {{id:'daily',name:'🏠 Vie Quotidienne'}},
  {{id:'prestige',name:'🏆 Prestige'}},
];
const BADGES=[
  {{id:'mineur',name:'Mineur',icon:'⛏',cat:'mining',
   tiers:[1000,10000,50000,100000],val:p=>p.total_mined}},
  {{id:'diamantaire',name:'Diamantaire',icon:'💎',cat:'mining',
   tiers:[10,50,150,300],val:p=>p.badge_data?.diamond_ore||0}},
  {{id:'nether_mole',name:'Nether Mole',icon:'🕳',cat:'mining',
   tiers:[1000,10000,20000,50000],val:p=>p.badge_data?.netherrack||0}},
  {{id:'ancient_debris',name:'Débris Anciens',icon:'🟫',cat:'mining',
   tiers:[5,20,50,100],val:p=>p.badge_data?.ancient_debris||0}},
  {{id:'bucheron',name:'Bûcheron',icon:'🪓',cat:'mining',
   tiers:[100,500,2000,5000],val:p=>p.badge_data?.logs||0}},
  {{id:'chasseur',name:'Chasseur',icon:'⚔',cat:'combat',
   tiers:[100,1000,10000,50000],val:p=>p.mob_kills}},
  {{id:'ender_slayer',name:'Ender Slayer',icon:'👁',cat:'combat',
   tiers:[100,1000,10000,40000],val:p=>p.badge_data?.enderman||0}},
  {{id:'nether_warrior',name:'Nether Warrior',icon:'🔥',cat:'combat',
   tiers:[50,200,500,1000],val:p=>(p.badge_data?.wither_skeleton||0)+(p.badge_data?.blaze||0)}},
  {{id:'berserker',name:'Berserker',icon:'💥',cat:'combat',
   tiers:[1000,10000,50000,100000],val:p=>Math.round(p.damage_dealt/20)}},
  {{id:'pvp_champion',name:'PvP Champion',icon:'🗡',cat:'combat',
   tiers:[1,5,15,30],val:p=>p.player_kills}},
  {{id:'raid_master',name:'Raid Master',icon:'🏴',cat:'combat',
   tiers:[10,50,100,200],val:p=>(p.badge_data?.pillager||0)+(p.badge_data?.vindicator||0)+(p.badge_data?.ravager||0)}},
  {{id:'increvable',name:'Increvable',icon:'🛡',cat:'survival',
   tiers:[2,5,10,20],val:p=>p.deaths>0?Math.round(p.play_hours/p.deaths*10)/10:(p.play_hours>=1?999:0)}},
  {{id:'kamikaze',name:'Kamikaze',icon:'💀',cat:'survival',
   tiers:[10,30,75,150],val:p=>p.deaths}},
  {{id:'bouclier_humain',name:'Bouclier Humain',icon:'🎯',cat:'survival',
   tiers:[5,15,30,50],val:p=>p.killed_by?.player||0}},
  {{id:'punching_bag',name:'Punching Bag',icon:'🥊',cat:'survival',
   tiers:[500,2000,5000,10000],val:p=>Math.round(p.damage_taken/20)}},
  {{id:'globe_trotter',name:'Globe-trotter',icon:'🌍',cat:'exploration',
   tiers:[50,200,500,1500],val:p=>p.total_distance_km}},
  {{id:'marathonien',name:'Marathonien',icon:'🏃',cat:'exploration',
   tiers:[42,100,200,500],val:p=>p.distances?.walk||0}},
  {{id:'sprinter',name:'Sprinter',icon:'💨',cat:'exploration',
   tiers:[50,150,300,500],val:p=>p.distances?.sprint||0}},
  {{id:'aviateur',name:'Aviateur',icon:'🪂',cat:'exploration',
   tiers:[50,200,500,1000],val:p=>p.distances?.aviate||0}},
  {{id:'marin',name:'Marin',icon:'⛵',cat:'exploration',
   tiers:[1,5,10,20],val:p=>p.distances?.boat||0}},
  {{id:'cavalier',name:'Cavalier',icon:'🐴',cat:'exploration',
   tiers:[1,5,15,30],val:p=>p.distances?.horse||0}},
  {{id:'fermier',name:'Fermier',icon:'🐄',cat:'farming',
   tiers:[10,50,200,1000],val:p=>p.animals_bred}},
  {{id:'pecheur',name:'Pêcheur',icon:'🎣',cat:'farming',
   tiers:[5,25,75,150],val:p=>p.fish_caught}},
  {{id:'commercant',name:'Commerçant',icon:'🤝',cat:'farming',
   tiers:[50,200,1000,3000],val:p=>p.traded_with_villager}},
  {{id:'recolte',name:'Récolte',icon:'🌾',cat:'farming',
   tiers:[100,500,2000,5000],val:p=>p.badge_data?.crops||0}},
  {{id:'artisan',name:'Artisan',icon:'🔨',cat:'craft',
   tiers:[1000,10000,30000,80000],val:p=>p.total_crafted}},
  {{id:'enchanteur',name:'Enchanteur',icon:'✨',cat:'craft',
   tiers:[10,50,200,500],val:p=>p.enchant_item}},
  {{id:'paperasse',name:'Paperasse',icon:'📜',cat:'craft',
   tiers:[100,1000,5000,15000],val:p=>p.badge_data?.paper||0}},
  {{id:'forgeron',name:'Forgeron',icon:'🔧',cat:'craft',
   tiers:[5,15,30,60],val:p=>p.badge_data?.total_broken||0}},
  {{id:'rat_de_coffre',name:'Rat de coffre',icon:'📦',cat:'daily',
   tiers:[100,1000,3000,5000],val:p=>p.open_chest}},
  {{id:'dormeur',name:'Dormeur',icon:'🛏',cat:'daily',
   tiers:[10,50,150,300],val:p=>p.sleep_in_bed}},
  {{id:'kangourou',name:'Kangourou',icon:'🦘',cat:'daily',
   tiers:[5000,20000,50000,80000],val:p=>p.jumps}},
  {{id:'no_life',name:'No-Life',icon:'🎮',cat:'prestige',
   tiers:[10,50,100,200],val:p=>p.play_hours}},
];
const BADGE_DESCS={{
  mineur:'Miner des blocs au total',
  diamantaire:'Miner du minerai de diamant',
  nether_mole:'Miner de la netherrack',
  ancient_debris:'Miner des débris antiques',
  bucheron:'Couper des bûches (tous types)',
  chasseur:'Tuer des mobs',
  ender_slayer:'Tuer des Endermen',
  nether_warrior:'Tuer des Wither Skeletons & Blazes',
  berserker:'Infliger des dégâts (en cœurs)',
  pvp_champion:'Tuer d\\'autres joueurs',
  raid_master:'Tuer Pillagers, Vindicators & Ravagers',
  increvable:'Ratio heures jouées / morts',
  kamikaze:'Mourir de nombreuses fois',
  bouclier_humain:'Se faire tuer par d\\'autres joueurs',
  punching_bag:'Encaisser des dégâts (en cœurs)',
  globe_trotter:'Parcourir des km au total',
  marathonien:'Marcher en km',
  sprinter:'Sprinter en km',
  aviateur:'Voler en Elytra en km',
  marin:'Naviguer en bateau en km',
  cavalier:'Chevaucher en km',
  fermier:'Élever des animaux',
  pecheur:'Pêcher des poissons',
  commercant:'Échanger avec des villageois',
  recolte:'Récolter blé, betteraves, carottes & patates',
  artisan:'Crafter des items au total',
  enchanteur:'Enchanter des items',
  paperasse:'Crafter du papier',
  forgeron:'Casser des outils à force d\\'usage',
  rat_de_coffre:'Ouvrir des coffres',
  dormeur:'Dormir dans un lit',
  kangourou:'Faire des sauts',
  no_life:'Accumuler des heures de jeu',
  all_rounder:'Bronze dans chaque catégorie',
  legende:'Obtenir Or ou mieux sur des badges',
}};
...
function computePlayerBadges(p){{
  ...
  results.push({{id:'all_rounder',name:'All-Rounder',icon:'🌟',cat:'prestige',tiers:arT,value:completeCats,tier:arTier,progress:arProg,nextTarget:arNext}});
  ...
  results.push({{id:'legende',name:'Légende',icon:'👑',cat:'prestige',tiers:lgT,value:goldCount,tier:lgTier,progress:lgProg,nextTarget:lgNext}});
  return results;
}}
...
function buildBadgesHtml(name){{
  ...
  let h=`<div class="card"><h3><span class="icon">🏅</span> Badges & Achievements</h3>`;
  h+=`<div class="badges-counter-wrap"><span class="badges-counter"><b>${{unlocked}}</b> / ${{total}} badges débloqués</span></div>`;
  BADGE_CATEGORIES.forEach(cat=>{{
    const cb=badges.filter(b=>b.cat===cat.id);
    if(!cb.length)return;
    h+=`<div class="badges-cat-header">${{cat.name}}</div><div class="badges-grid">`;
    cb.forEach(b=>{{
      const tn=TIER_NAMES[b.tier];
      const tl=b.tier>0?TIER_LABELS[b.tier]:'🔒';
      ...
      const desc=BADGE_DESCS[b.id]||'';
      const ttTiers=b.tiers.map((t,i)=>{{const cls=i<b.tier?'tt-done':(i===b.tier&&b.tier<4?'tt-next':'');return `<span class="tt-tier ${{cls}}">${{TIER_EMOJIS[i]}} ${{fmt(t)}}</span>`}}).join(' · ');
...
function buildPlayerSection(name){{
  ...
  const funFactsHtml=funFacts.length?funFacts.map(f=>`<div class="fun-fact"><span class="emoji">${{f.emoji}}</span><span>${{f.text}}</span></div>`).join(''):'<div style="color:var(--text-muted);font-family:var(--font-mono);font-size:.8rem">Pas assez de données</div>';
  ...
  <div class="section" id="player-${{name}}">
    ...
    <div class="grid grid-4" style="margin-bottom:1rem">
      <div class="stat-tile"><div class="value" style="color:var(--green)" data-target="${{p.total_mined}}">0</div><div class="label">Blocs minés</div><div class="sub">${{fmt(mph)}}/h</div></div>
      <div class="stat-tile"><div class="value" style="color:var(--red)" data-target="${{p.mob_kills}}">0</div><div class="label">Mobs tués</div><div class="sub">${{fmt(kph)}}/h</div></div>
      <div class="stat-tile"><div class="value" style="color:var(--orange)" data-target="${{p.deaths}}">0</div><div class="label">Morts</div><div class="sub">⚔ ${{pvpDeaths}} PvP · 🧟 ${{pveDeaths}} PvE</div></div>
      <div class="stat-tile"><div class="value" style="color:var(--cyan)" data-target="${{p.total_crafted}}">0</div><div class="label">Items craftés</div></div>
    </div>
    <div class="grid grid-4" style="margin-bottom:1rem">
      <div class="stat-tile"><div class="value" style="color:var(--yellow)" data-target="${{p.enchant_item}}">0</div><div class="label">Enchantements</div></div>
      <div class="stat-tile"><div class="value" style="color:var(--blue)" data-target="${{p.open_chest}}">0</div><div class="label">Coffres ouverts</div></div>
      <div class="stat-tile"><div class="value" style="color:var(--teal)" data-target="${{p.fish_caught}}">0</div><div class="label">Poissons pêchés</div></div>
    ...
    <div class="grid grid-2">
      <div class="card"><h3><span class="icon">🔧</span> Outils cassés</h3>${{buildBrokenHtml(p.broken)}}</div>
      <div class="card"><h3><span class="icon">💡</span> Fun Facts</h3><div class="fun-facts">${{funFactsHtml}}</div></div>
    </div>
