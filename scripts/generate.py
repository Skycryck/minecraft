...
nav_overview:'📊 Vue globale',nav_leaderboards:'🏆 Classements',
...
lb_playtime:'⏱ Temps de jeu',lb_mined:'⛏ Blocs minés',lb_kills:'⚔ Mobs tués',
lb_deaths:'💀 Morts',lb_distance:'🗺 Distance',lb_crafted:'🔨 Items craftés',
lb_pvp:'🗡 PvP Kills',lb_enchant:'✨ Enchantements',lb_fish:'🐟 Poissons',
lb_trades:'🤝 Échanges PNJ',lb_breed:'💕 Élevage',lb_jumps:'🦘 Sauts',
...
tier_locked:'🔒',tier_bronze:'🥉 Bronze',tier_silver:'🥈 Argent',tier_gold:'🥇 Or',tier_diamond:'💎 Diamant',
cat_mining:'⛏ Minage',cat_combat:'⚔ Combat',cat_survival:'💀 Survie',
cat_exploration:'🚶 Exploration',cat_farming:'🌾 Farming & Économie',
cat_craft:'🔨 Craft & Technique',cat_daily:'🏠 Vie Quotidienne',cat_prestige:'🏆 Prestige',
...
nav_overview:'📊 Overview',nav_leaderboards:'🏆 Leaderboards',
...
lb_playtime:'⏱ Playtime',lb_mined:'⛏ Blocks mined',lb_kills:'⚔ Mobs killed',
lb_deaths:'💀 Deaths',lb_distance:'🗺 Distance',lb_crafted:'🔨 Items crafted',
lb_pvp:'🗡 PvP Kills',lb_enchant:'✨ Enchantments',lb_fish:'🐟 Fish caught',
lb_trades:'🤝 NPC trades',lb_breed:'💕 Breeding',lb_jumps:'🦘 Jumps',
...
tier_locked:'🔒',tier_bronze:'🥉 Bronze',tier_silver:'🥈 Silver',tier_gold:'🥇 Gold',tier_diamond:'💎 Diamond',
cat_mining:'⛏ Mining',cat_combat:'⚔ Combat',cat_survival:'💀 Survival',
cat_exploration:'🚶 Exploration',cat_farming:'🌾 Farming & Economy',
cat_craft:'🔨 Craft & Tech',cat_daily:'🏠 Daily life',cat_prestige:'🏆 Prestige',
...
function getArchetype(p){{
  if(p.play_hours<1)return{{name:t('arch_new'),icon:'🌱',color:'var(--text-muted)'}};
  ...
  const types={{
    miner:{{name:t('arch_miner'),icon:'⛏',color:'var(--yellow)'}},
    fighter:{{name:t('arch_fighter'),icon:'⚔',color:'var(--red)'}},
    explorer:{{name:t('arch_explorer'),icon:'🧭',color:'var(--green)'}},
    builder:{{name:t('arch_builder'),icon:'🏗',color:'var(--cyan)'}},
    farmer:{{name:t('arch_farmer'),icon:'🌾',color:'var(--orange)'}}
  }};
...
function getFunFacts(name,p){{
  ...
  if(ek>50)facts.push({{score:ek/_maxOf(p=>p.killed_top10?.enderman||0),emoji:'👁',text:t('ff_endermen',fmt(ek),Math.floor(ek/64))}});

  if(p.deaths>3&&h>1){{const mpd=Math.round(h*60/p.deaths);const deathRate=p.deaths/h;
    facts.push({{score:deathRate/_maxOf(p=>p.play_hours>1?p.deaths/p.play_hours:0),emoji:'💀',text:t('ff_death_rate',mpd)}})}}

  const wk=p.distances?.walk||0;
  if(wk>20)facts.push({{score:wk/_maxOf(p=>p.distances?.walk||0),emoji:'🏃',text:t('ff_walk',wk.toFixed(0),(wk/42.195).toFixed(1))}});

  if(p.jumps>500&&h>1)facts.push({{score:p.jumps/_maxOf(p=>p.jumps||0),emoji:'🦘',text:t('ff_jumps',Math.round(p.jumps/h))}});

  if(p.total_mined>500&&h>1)facts.push({{score:p.total_mined/_maxOf(p=>p.total_mined||0),emoji:'⛏',text:t('ff_mining',(p.total_mined/(h*60)).toFixed(1))}});

  if(p.sleep_in_bed>5)facts.push({{score:p.sleep_in_bed/_maxOf(p=>p.sleep_in_bed||0),emoji:'🛏',text:t('ff_sleep',p.sleep_in_bed,(p.sleep_in_bed/h).toFixed(1))}});

  if(p.open_chest>100)facts.push({{score:p.open_chest/_maxOf(p=>p.open_chest||0),emoji:'📦',text:t('ff_chests',fmt(p.open_chest))}});

  const pvpD=(p.killed_by?.player)||0;
  if(pvpD>3)facts.push({{score:pvpD/_maxOf(p=>(p.killed_by?.player)||0),emoji:'🎯',text:t('ff_pvp_target',pvpD)}});

  const ely=p.distances?.aviate||0;
  if(ely>50)facts.push({{score:ely/_maxOf(p=>p.distances?.aviate||0),emoji:'🪂',text:t('ff_elytra',ely.toFixed(0),Math.round(ely/6))}});

  if(p.damage_dealt>10000)facts.push({{score:p.damage_dealt/_maxOf(p=>p.damage_dealt||0),emoji:'💔',text:t('ff_damage',fmt(Math.round(p.damage_dealt/20)))}});

  if(p.traded_with_villager>50)facts.push({{score:p.traded_with_villager/_maxOf(p=>p.traded_with_villager||0),emoji:'🤝',text:t('ff_trades',fmt(p.traded_with_villager))}});

  const brk=Object.values(p.broken||{{}}).reduce((s,v)=>s+v,0);
  if(brk>5)facts.push({{score:brk/_maxOf(p=>Object.values(p.broken||{{}}).reduce((s,v)=>s+v,0)),emoji:'💥',text:t('ff_tools',brk)}});

  if(p.mob_kills>100)facts.push({{score:p.mob_kills/_maxOf(p=>p.mob_kills||0),emoji:'⚔',text:t('ff_mob_kills',fmt(p.mob_kills),Math.round(p.mob_kills/h))}});

  if(p.animals_bred>20)facts.push({{score:p.animals_bred/_maxOf(p=>p.animals_bred||0),emoji:'💕',text:t('ff_breeding',p.animals_bred)}});

  if(p.fish_caught>10)facts.push({{score:p.fish_caught/_maxOf(p=>p.fish_caught||0),emoji:'🎣',text:t('ff_fishing',p.fish_caught)}});

  const totalDist=p.total_distance_km;
  if(totalDist>100)facts.push({{score:totalDist/_maxOf(p=>p.total_distance_km||0),emoji:'🌍',text:t('ff_total_dist',totalDist.toFixed(0),totalDist>1000?t('ff_equiv_long'):t('ff_equiv_short'))}});
...
function buildOverview(){{
  ...
  <div class="section active" id="overview">
    ...
    <div class="grid grid-2">
      <div class="card"><h3><span class="icon">⏱</span> ${{t('chart_playtime')}}</h3><div class="chart-wrap"><canvas id="chart-playtime"></canvas></div></div>
      <div class="card"><h3><span class="icon">🗺</span> ${{t('chart_distance')}}</h3><div class="chart-wrap"><canvas id="chart-distance"></canvas></div></div>
      <div class="card"><h3><span class="icon">⛏</span> ${{t('chart_mined')}}</h3><div class="chart-wrap"><canvas id="chart-mined"></canvas></div></div>
      <div class="card"><h3><span class="icon">⚔</span> ${{t('chart_kills')}}</h3><div class="chart-wrap"><canvas id="chart-kills"></canvas></div></div>
    </div>
...
function buildLeaderboards(){{
  ...
  h+=`</div>
    <div class="grid grid-2" style="margin-top:1rem">
      <div class="card"><h3><span class="icon">💀</span> ${{t('chart_deathcauses')}}</h3><div class="chart-wrap"><canvas id="chart-deathcauses"></canvas></div></div>
      <div class="card"><h3><span class="icon">🚶</span> ${{t('chart_dist_type')}}</h3><div class="chart-wrap"><canvas id="chart-dist-stacked"></canvas></div></div>
    </div></div>`;
  return h;
}}
...
function buildBadgesHtml(name){{
  ...
  let h=`<div class="card"><h3><span class="icon">🏅</span> ${{t('badges_title')}}</h3>`;
...
function buildPlayerSection(name){{
  ...
  const funFactsHtml=funFacts.length?funFacts.map(f=>`<div class="fun-fact"><span class="emoji">${{f.emoji}}</span><span>${{f.text}}</span></div>`).join(''):'<div style="color:var(--text-muted);font-family:var(--font-mono);font-size:.8rem">'+t('no_data')+'</div>';
  ...
  <div class="section" id="player-${{name}}">
    ...
    <div class="grid grid-4" style="margin-bottom:1rem">
      ...
      <div class="stat-tile"><div class="value" style="color:var(--orange)" data-target="${{p.deaths}}">0</div><div class="label">${{t('deaths')}}</div><div class="sub">⚔ ${{pvpDeaths}} ${{t('pvp')}} · 🧟 ${{pveDeaths}} ${{t('pve')}}</div></div>
    ...
    <div class="grid grid-2">
      <div class="card"><h3><span class="icon">🕐</span> ${{t('card_time_est')}}</h3><div class="chart-wrap"><canvas id="chart-time-${{name}}"></canvas></div></div>
      <div class="card"><h3><span class="icon">🚶</span> ${{t('card_distances')}}</h3><div class="chart-wrap"><canvas id="chart-dist-${{name}}"></canvas></div></div>
    </div>
    <div class="grid grid-2">
      <div class="card"><h3><span class="icon">💀</span> ${{t('card_killed_by')}}</h3><ul class="leaderboard" style="font-size:.8rem">${{kbHtml}}</ul></div>
    </div>
    <div class="card desktop-only">
      <h3><span class="icon">⛏</span> ${{t('card_treemap')}}</h3>
      ${{buildTreemapHtml(Object.entries(p.mined_top15||{{}}))}}
    </div>
    <div class="card mobile-only">
      <h3><span class="icon">⛏</span> ${{t('card_top15_mined')}}</h3>
      <ol class="leaderboard">${{mkList(Object.entries(p.mined_top15||{{}}),color)}}</ol>
    </div>
    <div class="grid grid-2">
      <div class="card"><h3><span class="icon">⚔</span> ${{t('card_top10_killed')}}</h3><ol class="leaderboard">${{mkList(Object.entries(p.killed_top10||{{}}),'var(--red)')}}</ol></div>
      <div class="card"><h3><span class="icon">🔨</span> ${{t('card_top10_crafted')}}</h3><ol class="leaderboard">${{mkList(Object.entries(p.crafted_top15||{{}}).slice(0,10),'var(--cyan)')}}</ol></div>
    </div>
    <div class="grid grid-2">
      <div class="card"><h3><span class="icon">🔧</span> ${{t('card_tools_broken')}}</h3>${{buildBrokenHtml(p.broken)}}</div>
      <div class="card"><h3><span class="icon">💡</span> ${{t('card_fun_facts')}}</h3><div class="fun-facts">${{funFactsHtml}}</div></div>
    </div>
