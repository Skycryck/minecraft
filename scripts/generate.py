...
function getFunFacts(name,p){{
  ...
  if(ek>100)facts.push({{emoji:'👁',text:`A tué ${{fmt(ek)}} Endermen — environ ${{Math.floor(ek/64)}} stacks d'Ender Pearls`}});
  if(p.deaths>5&&h>1){{const mpd=Math.round(h*60/p.deaths);facts.push({{emoji:'💀',text:`Meurt en moyenne toutes les ${{mpd}} minutes`}})}}
  const wk=p.distances?.walk||0;
  if(wk>50)facts.push({{emoji:'🏃',text:`A marché ${{wk.toFixed(0)}} km — soit ${{(wk/42.195).toFixed(1)}} marathons`}});
  if(p.jumps>1000&&h>1)facts.push({{emoji:'🦘',text:`Saute ${{Math.round(p.jumps/h)}} fois par heure en moyenne`}});
  if(p.total_mined>1000&&h>1)facts.push({{emoji:'💎',text:`Mine ${{(p.total_mined/(h*60)).toFixed(1)}} blocs par minute en moyenne`}});
  if(p.sleep_in_bed>10)facts.push({{emoji:'🛏',text:`A dormi ${{p.sleep_in_bed}} nuits Minecraft — ${{(p.sleep_in_bed/h).toFixed(1)}} par heure de jeu`}});
  if(p.open_chest>1000)facts.push({{emoji:'📦',text:`A ouvert ${{fmt(p.open_chest)}} coffres — un vrai fouineur`}});
  const pvpD=(p.killed_by?.player)||0;
  if(pvpD>10)facts.push({{emoji:'🎯',text:`Tué ${{pvpD}} fois par d'autres joueurs — cible favorite du serveur`}});
  const ely=p.distances?.aviate||0;
  if(ely>100)facts.push({{emoji:'🪂',text:`${{ely.toFixed(0)}} km en Elytra — l'équivalent de ${{Math.round(ely/6)}} traversées de Paris`}});
  if(p.damage_dealt>50000)facts.push({{emoji:'💔',text:`A infligé ${{fmt(Math.round(p.damage_dealt/20))}} cœurs de dégâts au total`}});
  if(p.traded_with_villager>500)facts.push({{emoji:'🤝',text:`${{fmt(p.traded_with_villager)}} échanges avec des villageois — le capitaliste du serveur`}});
  const brk=Object.values(p.broken||{{}}).reduce((s,v)=>s+v,0);
  if(brk>20)facts.push({{emoji:'💥',text:`A cassé ${{brk}} outils — pas très soigneux`}});
